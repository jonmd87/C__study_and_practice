...
void	print_info(const struct data * pnt)
{
		int		ind;
		...
		while (ind < V)
		{
			if (isalpha(pnt[ind].person.middle[0]))
				printf("%s, %s  %c.  -- %-25i\n",
						 pnt[ind].person.last, pnt[ind].person.first, pnt[ind].person.middle[0], pnt[ind].ssn);
			else
				printf("%5s, %s %c  -- %i\n",
						 pnt[ind].person.last, pnt[ind].person.first, ' ', pnt[ind].ssn);
			ind++;
		}
}
